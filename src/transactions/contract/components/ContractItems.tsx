  ...
  return (
    <div className="mt-8">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-medium">Contract Items</h2>
        <button
          type="button"
          onClick={onAddItem}
      ...
      {/* Contract Items Preview */}
      {items.length > 0 && (
        <div className="mt-4">
          <span className="text-sm text-gray-700">
            {items.map(item => `${item.product_id || 'Item name not entered'}(${item.quality}) ${item.quantity} x ${item.unit_price} KRW`).join(', ')}
          </span>
        </div>
      )}
    </div>
