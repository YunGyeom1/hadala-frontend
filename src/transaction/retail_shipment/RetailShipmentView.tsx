...
import type { RetailShipment, RetailShipmentItem } from './retail_shipment';
...
export const RetailShipmentView = ({ shipment, onUpdate, onDelete }: RetailShipmentViewProps) => {
  ...
  const handleAddItem = () => {
    const newItem: RetailShipmentItem = {
      id: `temp-${Date.now()}`,
      ...
      <div className="bg-white shadow rounded-lg overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-gray-200">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            <div>
              <h3 className="text-sm font-medium text-gray-500">출하명</h3>
              {isEditing ? (
                <input
                  type="text"
                  value={editedShipment.shipment_name}
                  onChange={(e) => setEditedShipment(prev => ({ ...prev, shipment_name: e.target.value }))}
                  className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                />
              ) : (
                <p className="mt-1 text-sm text-gray-900">{editedShipment.shipment_name}</p>
              )}
            </div>
            <div>
            ...
            <div>
              ...
              {isEditing ? (
                <input
                  type="text"
                  value={editedShipment.retailer_id}
                  onChange={(e) => setEditedShipment(prev => ({ ...prev, retailer_id: e.target.value }))}
                  className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                />
              ) : (
                <p className="mt-1 text-sm text-gray-900">{editedShipment.retailer_id}</p>
              )}
            </div>
            <div>
