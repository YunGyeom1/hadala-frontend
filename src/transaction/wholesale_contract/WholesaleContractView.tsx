...
import type { WholesaleContract, WholesaleContractItem } from './wholesale_contract';
...
export const WholesaleContractView = ({ contract, onUpdate, onDelete }: WholesaleContractViewProps) => {
  const [editedContract, setEditedContract] = useState<WholesaleContract>(contract);
  ...
  const handleAddItem = () => {
    const newItem: WholesaleContractItem = {
      id: `temp-${Date.now()}`,
  ...
  const getStatusColor = (status: ContractStatus | PaymentStatus) => {
    switch (status) {
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            <div>
              ...
              {isEditing ? (
                <input
                  type="text"
                  value={editedContract.farmer_id}
                  onChange={(e) => setEditedContract(prev => ({ ...prev, farmer_id: e.target.value }))}
                  className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                />
              ) : (
                <p className="mt-1 text-sm text-gray-900">{editedContract.farmer_id}</p>
              )}
            </div>
            <div>
            ...
            <div>
              ...
              {isEditing ? (
                <select
                  ...
                  onChange={(e) => setEditedContract(prev => ({ ...prev, contract_status: e.target.value as ContractStatus }))}
                  className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                >
                  <option value={ContractStatus.PENDING}>대기중</option>
                  <option value={ContractStatus.APPROVED}>승인</option>
                  <option value={ContractStatus.REJECTED}>거절</option>
                  <option value={ContractStatus.COMPLETED}>완료</option>
                </select>
              ) : (
            ...
            <div>
              ...
              {isEditing ? (
                <select
                  ...
                  onChange={(e) => setEditedContract(prev => ({ ...prev, payment_status: e.target.value as PaymentStatus }))}
                  className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                >
                  <option value={PaymentStatus.PENDING}>대기중</option>
                  <option value={PaymentStatus.PAID}>결제완료</option>
                  <option value={PaymentStatus.OVERDUE}>연체</option>
                </select>
              ) : (
            ...
            <div>
              ...
              {isEditing ? (
                <input
                  type="date"
                  value={editedContract.shipment_date.split('T')[0]}
                  onChange={(e) => setEditedContract(prev => ({ ...prev, shipment_date: e.target.value }))}
                  className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                />
              ) : (
                <p className="mt-1 text-sm text-gray-900">{new Date(editedContract.shipment_date).toLocaleDateString('ko-KR')}</p>
              )}
            </div>
          </div>
        </div>
